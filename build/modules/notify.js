...
async function notifyOneGuild(g, opts) {
    ...
    return !announce
        ? Promise.all(entries.map(({ discordCh, roleToNotify }) => {
            const ch = guildObj?.channels.cache.find((ch) => ch.id === discordCh);
            const msgPromise = (0, discord_1.send)(ch, {
                content: roleToNotify ? emoji + ' <@&' + roleToNotify + '>' : undefined,
                embeds: [
                    (0, discord_1.createEmbed)({
            ...
            if (opts.nonEmbedText)
                (0, discord_1.send)(ch, opts.nonEmbedText);
            return msgPromise;
        }))
        : [];
}
