...
export async function sendAndForgetHistory(videoId: VideoId): Promise<void> {
  const allRelayHistories = await getAllRelayHistories();
  const relevantHistories = allRelayHistories
  ...
  relevantHistories.forEach(async (history: RelayedComment[], gid) => {
    ...
    const tlLog = await filterAndStringifyHistory(gid, history, start);
