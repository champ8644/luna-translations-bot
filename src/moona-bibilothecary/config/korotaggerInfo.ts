...
class KorotaggerInfo {
  ...
  async setLog(message: Message): Promise<void> {
    ...
    if (!channel?.isText()) throw 'This channel is not text-based, please try again.';
    ...
    await message.reply(`\`\`\`Set current channel "${channelId}" as log posting location\`\`\``);
  }
  ...
  async addListener(message: Message, args: Array<string>): Promise<void> {
    ...
    await SQL.insertInto('channel_info', ['botId', 'channelId', 'type'], payload);
    this.listen.push({ botId: payload.botId, channelId: payload.channelId });

    await message.reply(`\`\`\`Listening to korotag output from "${name}"\`\`\``);
  }
  ...
  async clearListener(message: Message, args: Array<string>): Promise<void> {
    ...
    if (payload.botId) {
      ...
      this.listen = this.listen.filter(
        (each) => each.botId !== payload.botId || each.channelId !== payload.channelId,
      );
      ...
      await SQL.deleteFromEqual('channel_info', payload);
      this.listen = this.listen.filter((each) => each.channelId !== payload.channelId);
