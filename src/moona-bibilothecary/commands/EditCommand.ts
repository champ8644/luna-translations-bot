...
export class EditCommand extends Command implements CommandInterface {
  ...
  async awaitReplyDate(message: Message, timestamp: number | null): Promise<Message | undefined> {
  ...
  async awaitReplyLine(message: Message, found: LineComponent): Promise<Message | undefined> {
    const messageReply = await message.reply(
      `Reply to this message with the message to edit, or press ${
        moonaEmoji.denied
  ...
  awaitReply(message: Message, messageReply: Message): Promise<Message | undefined> {
  ...
  ): Promise<{ type: 'accept'; output: number } | { type: 'cancel' } | { type: 'edit' }> {
  ...
  showConfirmationLine(
    ...
    originalMessage: Message,
    found: LineComponent,
  ): Promise<{ type: 'accept'; output: LineComponent } | { type: 'cancel' } | { type: 'edit' }> {
  ...
  async showConfirmation<T extends LineComponent | number | null>(
    ...
    output: T,
  ): Promise<{ type: 'accept'; output: T } | { type: 'cancel' } | { type: 'edit' }> {
    return new Promise(async (resolve, reject) => {
      try {
  ...
  async editOriginalEmbedDate(payload: { originalMessage: Message; timestamp: number }) {
    ...
    if (res) {
      ...
      const finalAuthor =
        prefix + ' ' + formatInTimeZone(fromUnixTime(timestamp), 'Asia/Bangkok', 'dd/MM/yy');
    ...
    if (fetchedOriginalMessage.hasThread) {
      ...
      await fetchedOriginalMessage.thread.setName(
        prefixThread +
          ' - ' +
          formatInTimeZone(fromUnixTime(timestamp), 'Asia/Bangkok', 'dd MMM yyyy'),
      );
          ...
          switch (typeEditing) {
            case 'date': {
              ...
              do {
                ...
                const messageAnswerReply = await this.awaitReplyDate(message, timestamp);
            ...
            default: {
              ...
              do {
                ...
                // await message.reply({ embeds });
                // await reply from message
                const messageAnswerReply = await this.awaitReplyLine(message, found);
                // User answered the edited answer, display the confirmation message
                if (messageAnswerReply) {
