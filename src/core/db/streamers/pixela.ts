export const pixela = [
  {
    name: 'Pixela',
    ytId: 'UCcRaKGCG3RFenb8D2php_Jw',
    chName: 'Pixela Project',
    picture:
      'https://yt3.ggpht.com/ytc/AKedOLQC0VQs6ILs90dRfOaQegPDgG5_Quk4kDTCccDc=s800-c-k-c0xffffffff-no-rj-mo',
    groups: ['Pixela'],
    twitter: 'PixelaProject',
    aliases: [],
  },
  {
    name: 'HongFei',
    ytId: 'UCutz6S1DcEHPnEb_r9ztkzg',
    chName: 'Hinabe HongFei Ch. Pixela Project',
    picture:
      'https://yt3.ggpht.com/wEre22q2Kqttj52wc_xUeWUMXPI9yuwD0raj-VgQc-0O0JQPvI8puphp0YGXOKF0dzBCU4EN=s800-c-k-c0xffffffff-no-rj-mo',
    groups: ['Pixela'],
    twitter: 'HongFei_PIX',
    aliases: [],
  },
  {
    name: 'Zelina',
    ytId: 'UCOaTgKPjI9cgXLoDW7XFDDw',
    chName: 'Princess Zelina Ch. Pixela Project',
    picture:
      'https://yt3.ggpht.com/4pjAgNelRNBaYyN1NXsMSITrfLzKYodDQLdT1Uz5DwEQSzgNXrtS-sbT65ZZNZZYs8vmv1zYiw=s800-c-k-c0xffffffff-no-rj-mo',
    groups: ['Pixela'],
    twitter: 'Zelina_PIXv2',
    aliases: [],
  },
  {
    name: 'Melita',
    ytId: 'UCpNkVsMlsJRF792H-1_1vPA',
    chName: 'Melita X Ch. Pixela Project',
    picture:
      'https://yt3.ggpht.com/PWJGd-XIEKIr5cpxVmNrPg7YoXUvB1OOIzJXgwlfvuv3d8A2jDtGKAJFFpErmJ_xRxP0Hd1SQjw=s800-c-k-c0xffffffff-no-rj-mo',
    groups: ['Pixela'],
    twitter: 'Melita_PIX',
    aliases: [],
  },
  {
    name: 'JuJu',
    ytId: 'UCMUtWzsjAQJgp6UlkbVSo1w',
    chName: 'Laguna JuJu Ch. Pixela Project',
    picture:
      'https://yt3.ggpht.com/Inr_qbHrHrHJ4smKEUW_NE39_-tnXqHU3HNxNkP4E-a5zHy7_F0dlrBsz3TlJSmvJIn9rVZX=s800-c-k-c0xffffffff-no-rj-mo',
    groups: ['Pixela'],
    twitter: 'JuJu_PIX',
    aliases: [],
  },
] as const;
